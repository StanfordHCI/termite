...
var MatrixState = CoreModel.extend({
	...
	"url" : "default/state/TermTopicMatrix"
});
...
MatrixState.prototype.loadData = function() {
	$.ajax( "default/data/TermTopicMatrix", {
		"success" : function( results, responseMessage, responseObject ) {
			console.log( "[MatrixState.loadData] [success]", responseMessage, responseObject, results );
			var rowDims = results.terms.length;
			var columnDims = results.topics.length;
			this.importEntries( "identifier", results.matrix, rowDims, columnDims )
				.allRowLabels( results.terms )
				.allColumnLabels( results.topics )
				.setParameters( results.states );
		}.bind(this),
		"error" : function( results, responseMessage, responseObject ) {
			console.log( "[MatrixState.loadData] [error]", responseMessage, responseObject, results );
		}.bind(this)
	});
};
